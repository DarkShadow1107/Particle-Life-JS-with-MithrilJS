class App {
	...
	view() {
		return m(
			".flex.justify-end.items-center.h-full.app",
			this.loaded &&
				m(
					".absolute.left-0.p-3.w-72.h-full.overflow-auto.bg-black.bg-opacity-50.scrollbar-dark.controlPanel",
					{
						hidden: !this.isShowPanel,
					},
					m("p", "Interaction force:"),
					m(
						"table.w-full.table-fixed.rounded.overflow-hidden.text-center",
						m(
							"tr.h-7",
							m("th.font-normal", m("sub", "A"), m("sup", "B")),
						...
						this.groups.map((groupA) =>
							m(
								"tr.h-7",
								m(
									"th.justify-center.items-center.text-xs.font-normal.cursor-pointer",
									{
								...
								this.groups.map((groupB) => {
									...
									return m(
										"td.justify-center.items-center.text-xs.text-white.cursor-pointer",
										{
											style: {
						...
						this.groupA &&
							m(
								...
								m(
									...
									m(
										".ml-2.w-6.h-6.flex.justify-center.items-center.text-xs",
										{
											style: {
					...
					m(
						...
						m(
							"button.px-2.border.border-gray-400.hover:border-gray-200.rounded.bg-gray-600",
							{
								onclick: () => {
									this.sendCall("randomGMaps");
								},
					...
					m("p.mt-2", "Tips:"),
					m("p.text-sm.text-gray-400", m("div", "Open fullscreen for best viewing.")),

					m("p.mt-2", "Buy me a coffee 💖:"),
					m(
						"a.px-2.border.border-gray-400.hover:border-gray-200.rounded.bg-gray-600",
						{
							href: "https://buymeacoffee.com/darkshadow07",
							target: "_blank",
						},
						"Open"
					)
				),
			(this.canvasVnode = m("canvas.max-w-full.max-h-full", {
